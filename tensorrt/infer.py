# by yhpark 2025-7-28
# modified by TillBeemelmanns 2025-9-01
import json
import os
import time
...
def infer_performace(model, input_size=(518, 518)):
    ...
    with torch.no_grad():
        for _ in range(20):
            _ = model(dummy_input, num_tokens)
...
def set_model(encoder='vits', normal=True, dtype: torch.dtype = torch.float32):
    ...
    if normal:
        checkpoint = f"../checkpoint/moge-2-{encoder}-normal/model.pt"
    else:
        encoder='vitl'
        checkpoint = f"../checkpoint/moge-2-{encoder}/model.pt"
...
def main():
    fov_x = 74  # Set to desired FOV in degrees (e.g., 60.0) or None to auto-recover
    ...
    # input
    image_file_name = '0000.jpg'
    image_path = "../data/front_center/0000.jpg"
